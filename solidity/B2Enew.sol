...
contract B2Enew{
    ...
    function B2Enew(address _ingrid_eth_pk, bytes _bob_btc_pk, bytes _hash, uint _min, uint _rate, uint _deltaR, uint _bfund) public 
    {
        alice = msg.sender;
    ...
    function payin(uint blocktime, bytes _ingrid_btc_pk) public payable {
    ...
    function refresh(bytes _bob_btc_pk, bytes _hash, uint _min, uint _rate, uint _deltaR, uint _bfund) public{
    ...
    function check_output(bytes _bodyS) public returns (bool){
        ...
        bytes20 hash_ingrid_pk = ripemd160(sha256(ingrid_pubkey)); //20 bytes pk hash of Ingrid
